...
export const ProjectItem = ({ imagem, title, description, repository, deploy }: Props) => {
    return (
        <div className="max-w-96 bg-gray-950 rounded-2xl overflow-hidden">
            <img className="w-96 h-52 rounded-t-2xl object-cover max-md:w-full max-md:h-40" src={imagem} alt={title} />
            <h2 className="text-xl m-4 max-md:text-lg"> {title} </h2>
            <p className="text-sm text-gray-500 m-4 max-md:text-xs"> {description} </p>
